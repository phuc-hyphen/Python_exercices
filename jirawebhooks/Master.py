...
class JIRAMaster:
    def __init__(self, jira_connection, Slave):
    ...
    def IssueCreatedEvent(self):
        # self.GetIssue(data)
        # if "status" in self.issue_dict.keys():
        #     self.issue_dict.pop('status', None)
        new_issue = self.jira_connection.create_issue(fields=self.issue_dict)
        issue = self.jira_connection.issue(self.issue_key)
        issue.update(fields={self.External_ID_Field: new_issue.key})
        print(new_issue)

    def IssueModifiedEvent(self):
        
        issue = self.jira_connection.issue(self.issue_key)
        issue.update(fields=self.issue_dict)
        print("issue modified")
